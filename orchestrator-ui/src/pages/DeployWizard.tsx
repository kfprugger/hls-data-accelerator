...
export function DeployWizard() {
  ...
  const getEstimatedDurationMinutes = (): number => {
    ...
    if (config.scaffolding_only) minutes += 45; // HDS source artifact publication without pipeline runs
            ...
            <Card className={`${styles.section} ${styles.sectionFullWidth}`} style={{ overflow: "visible" }}>
              ...
              <div className={styles.fieldGroup}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: tokens.spacingHorizontalS }}>
                  ...
                  <Button appearance="outline" onClick={() => applyPreset("scaffold")}>Scaffolding / no data</Button>
                  ...
                  <div className={styles.fieldGroup}>
                    ...
                    {existingDeploy && (
                      <div style={{
                        ...
                        <div className={`deploy-collapsible-content${existingDeployCollapsed ? " deploy-collapsible-collapsed" : ""}`}
                          style={{ maxHeight: existingDeployCollapsed ? "0" : "640px" }}
                        >
                          ...
                          <div className={styles.dataStrategyControl}>
                            <Text weight="semibold">Existing FHIR data strategy</Text>
                            <RadioGroup
                              aria-label="Existing FHIR data strategy"
                              value={config.reseed_data ? "reseed" : config.reuse_patients ? "reuse" : ""}
                              onChange={(_, data) => update(data.value === "reseed" ? "reseed_data" : "reuse_patients", true)}
                              disabled={config.scaffolding_only}
                            >
                              <div className={styles.dataStrategyChoice}>
                                <Radio value="reuse" label="Keep and reuse current data" />
                                <Text size={200} className={styles.dataStrategyDescription}>
                              ...
                              <div className={styles.dataStrategyChoice}>
                                <Radio value="reseed" label="Reseed and replace data" />
                                <Text size={200} className={styles.dataStrategyDescription}>
                            ...
                            {config.reseed_data && (
                              <MessageBar intent="warning" className={styles.reseedWarning}>
                                <MessageBarBody>
                                  <Text weight="semibold">Permanent data replacement: </Text>
                    ...
                    >
                      <SpinButton
                        ...
                        disabled={config.scaffolding_only || config.reuse_patients || config.use_cached_synthea}
                      />
                    </Field>
                    <div style={{ marginTop: tokens.spacingVerticalS, display: "flex", flexDirection: "column", gap: tokens.spacingVerticalXS, marginBottom: tokens.spacingVerticalS }}>
                      <Checkbox
                        ...
                        label="Use cached canonical Synthea fixture (fixed at 100 patients)"
                      />
                  ...
                  <div className={styles.checkboxGroup}>
                    ...
                    <div className={`deploy-collapsible-content ${skipGroup1Collapsed ? "deploy-collapsible-collapsed" : ""}`} style={{ display: "flex", flexDirection: "column", gap: tokens.spacingVerticalXS, padding: "8px 12px 12px", transition: "all 0.3s ease" }}>
                      <Tooltip content="Skip Event Hub, ACR, emulator ACI, Storage, and Bicep infra" relationship="description" positioning="after">
                        <Checkbox
                          label={`${config.scaffolding_only ? "Azure Infrastructure Scaffold" : "Azure Synthetic Data Foundation"}${existingDeploy ? " (already deployed)" : ""}`}
                      ...
                      <div style={{ paddingLeft: 24, display: "flex", flexDirection: "column", gap: tokens.spacingVerticalXS }}>
                        <Tooltip content="Skip Synthea patient generation — use existing patients" relationship="description" positioning="after">
                          <Checkbox
                            ...
                            disabled={config.scaffolding_only || config.skip_fhir}
                          />
                        </Tooltip>
                        {!config.skip_synthea && (
                          <Tooltip content="Use the cached canonical fixture of exactly 100 patients instead of starting the generation container. Patient Count is locked to 100." relationship="description" positioning="after">
                            <Checkbox
                              label="Use cached canonical fixture (fixed at 100 patients)"
                              checked={config.use_cached_synthea}
                              onChange={(_, d) => update("use_cached_synthea", !!d.checked)}
                              disabled={config.skip_fhir || config.reuse_patients}
                              style={{ marginLeft: 24 }}
                            />
                          </Tooltip>
                        )}
                      ...
                      <Tooltip content="Skip TCIA DICOM download, re-tagging, ADLS upload, and FHIR ImagingStudy creation" relationship="description" positioning="after">
                        <Checkbox
                          ...
                          disabled={config.scaffolding_only || config.skip_base_infra}
                        />
                      </Tooltip>
                    </div>
                    ...
                    <div className={`deploy-collapsible-content ${skipGroup2Collapsed ? "deploy-collapsible-collapsed" : ""}`} style={{ display: "flex", flexDirection: "column", gap: tokens.spacingVerticalXS, padding: "8px 12px 12px", transition: "all 0.3s ease" }}>
                      ...
                      <div style={{ paddingLeft: 24 }}>
                        <Tooltip content="Skip FHIR $export to ADLS Gen2 — HDS pipelines need this data" relationship="description" positioning="after">
                          <Checkbox
                            ...
                            disabled={config.scaffolding_only || config.skip_fabric || config.skip_fhir}
                          />
                        </Tooltip>
                      </div>
                      <Tooltip content="Skip RTI Phase 2 — KQL→Silver shortcuts and enriched alert functions" relationship="description" positioning="after">
                        <Checkbox
                          ...
                          disabled={config.scaffolding_only || config.skip_fabric}
                        />
                      </Tooltip>
                    </div>
                    ...
                    <div className={`deploy-collapsible-content ${skipGroup3Collapsed ? "deploy-collapsible-collapsed" : ""}`} style={{ display: "flex", flexDirection: "column", gap: tokens.spacingVerticalXS, padding: "8px 12px 12px", transition: "all 0.3s ease" }}>
                      ...
                      <Tooltip content={`Skip Data Activator Reflex + email rule${!config.alert_email ? " (no alert email set)" : ""}`} relationship="description" positioning="after">
                        <Checkbox
                          ...
                          disabled={config.scaffolding_only || config.skip_fabric || !config.alert_email}
                        />
                      </Tooltip>
                    ...
                    <div className={`deploy-collapsible-content ${skipGroup4Collapsed ? "deploy-collapsible-collapsed" : ""}`} style={{ display: "flex", flexDirection: "column", gap: tokens.spacingVerticalXS, padding: "8px 12px 12px", transition: "all 0.3s ease" }}>
                      ...
                      <div style={{ paddingLeft: 24, display: "flex", flexDirection: "column", gap: tokens.spacingVerticalXS }}>
                        <Tooltip content={config.scaffolding_only ? "Deploy claim-stream, payer KQL scoring, and Eventstream definitions without the claim emulator" : "Deploy claim-stream, payer KQL scoring, claim emulator, and Eventstream extension"} relationship="description" positioning="after">
                          <Checkbox
                            label={config.scaffolding_only ? "Payer RTI scoring + claim stream (no producer)" : "Payer RTI scoring + claim stream"}
                        ...
                        <Tooltip content="Deploy PayerOpsActivator Reflex email alerts" relationship="description" positioning="after">
                          <Checkbox
                            ...
                            disabled={config.scaffolding_only || config.skip_phase7 || (!config.alert_email && !config.payer_ops_email)}
                          />
                        </Tooltip>
        ...
        >
          ...
          <Card>
            ...
            <div style={{ padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div>
                <Text size={200} weight="semibold" block>{config.reseed_data ? "Final Patient Count" : "Patient Count"}</Text>
                <Text size={200} block style={{ color: tokens.colorNeutralForeground2 }}>
