...
> A comprehensive architectural walkthrough of the `hls-data-accelerator` repository — a production-grade reference architecture that unifies healthcare EHR data and real-time medical device telemetry on Microsoft Fabric.
...
gantt
    ...
    HDS v1.4 Source Deployment    :crit, hds, after p1e, 45min

    section Stage 2: Active Patient Telemetry
    Fabric RTI Telemetry          :p1e, after p1c, 15min
    RTI Shortcuts                 :p2a, after hds, 15min
...
- Automated HDS source deployment with durable activity retries and observable validation
