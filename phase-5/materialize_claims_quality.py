...
try:
    ...
    patients = patient_df.select(
        ...
        F.col("gender"),
        F.floor(F.datediff(MEASUREMENT_AS_OF, F.col("birthDate").cast("date")) / 365.25).alias("age")
    ...
    patient_obs = observation_df.select(
        ...
        F.col("patient_id") != ""
    ).filter(F.col("obs_date").between(MEASUREMENT_YEAR_START, MEASUREMENT_YEAR_END))
    ...
    patient_imm = immunization_df.select(
        ...
        F.col("patient_id") != ""
    ).filter(F.col("imm_date").between(MEASUREMENT_YEAR_START, MEASUREMENT_YEAR_END))
    ...
    bmi_obs = patient_obs.filter(F.col("loinc_code") == "39156-5").select("patient_id").distinct().withColumn("has_bmi", F.lit(True))
    ...
    cms69_result = cms69_denom.join(bmi_obs, "patient_id", "left").withColumn(
        ...
        "in_numerator", F.coalesce(F.col("has_bmi"), F.lit(False))
    ).withColumn("in_exclusion", F.lit(False)).withColumn(
        "quality_met", F.coalesce(F.col("has_bmi"), F.lit(False))
    ).select("patient_id", "measure_id", "measure_name", "in_initial_population",
             "in_denominator", "in_numerator", "in_exclusion", "quality_met")
    ...
    ).select("patient_id").distinct().withColumn("has_pneumo", F.lit(True))
    ...
    cms127_result = cms127_denom.join(pneumo_patients, "patient_id", "left").withColumn(
        ...
        "in_numerator", F.coalesce(F.col("has_pneumo"), F.lit(False))
    ).withColumn("in_exclusion", F.lit(False)).withColumn(
        "quality_met", F.coalesce(F.col("has_pneumo"), F.lit(False))
    ).select("patient_id", "measure_id", "measure_name", "in_initial_population",
             "in_denominator", "in_numerator", "in_exclusion", "quality_met")
    ...
    ).select("patient_id").distinct().withColumn("has_flu", F.lit(True))
    ...
    cms147_result = cms147_denom.join(flu_patients, "patient_id", "left").withColumn(
        ...
        "in_numerator", F.coalesce(F.col("has_flu"), F.lit(False))
    ).withColumn("in_exclusion", F.lit(False)).withColumn(
        "quality_met", F.coalesce(F.col("has_flu"), F.lit(False))
    ).select("patient_id", "measure_id", "measure_name", "in_initial_population",
             "in_denominator", "in_numerator", "in_exclusion", "quality_met")
    ...
    nephro_screened = albumin_patients.union(acei_arb_patients).distinct().withColumn("has_nephro_screen", F.lit(True))
    ...
    cms134_result = cms134_denom.join(nephro_screened, "patient_id", "left").withColumn(
        ...
        "in_numerator", F.coalesce(F.col("has_nephro_screen"), F.lit(False))
    ).withColumn("in_exclusion", F.lit(False)).withColumn(
        "quality_met", F.coalesce(F.col("has_nephro_screen"), F.lit(False))
    ).select("patient_id", "measure_id", "measure_name", "in_initial_population",
             "in_denominator", "in_numerator", "in_exclusion", "quality_met")
    ...
    ).select("patient_id").distinct().withColumn("has_beta_blocker", F.lit(True))
    ...
    cms144_result = cms144_denom.join(bb_patients, "patient_id", "left").withColumn(
        ...
        "in_numerator", F.coalesce(F.col("has_beta_blocker"), F.lit(False))
    ).withColumn("in_exclusion", F.lit(False)).withColumn(
        "quality_met", F.coalesce(F.col("has_beta_blocker"), F.lit(False))
    ).select("patient_id", "measure_id", "measure_name", "in_initial_population",
             "in_denominator", "in_numerator", "in_exclusion", "quality_met")
    ...
    all_measures = all_measures.withColumn("measurement_year", F.lit(2026))
...
try:
    ...
    med_parsed = medication_df.select(
        ...
        (F.col("patient_id") != "") & F.col("authored_date").between(MEASUREMENT_YEAR_START, MEASUREMENT_YEAR_END)
    )
    ...
    # Simplified PDC: distinct year-months with an active prescription / 12.
    med_adherence = med_classified.groupBy("patient_id", "medication_class").agg(
        F.countDistinct(F.date_format(F.col("authored_date"), "yyyy-MM")).alias("months_with_rx"),
...
try:
    ...
    member_months_df = coverage_df.select(
        ...
        F.coalesce(F.get_json_object(F.col("period_string"), "$.end").cast("date"), MEASUREMENT_AS_OF).alias("cov_end")
    ).withColumn(
        "patient_id", F.regexp_extract(F.col("patient_ref"), r"Patient/(.*)", 1)
    ).withColumn(
...
for t in tables:
    try:
        ...
        if count <= 0 and t not in allow_empty_tables:
            validation_failures.append(f"{t} is empty but required")
            print(f"    ✗ Empty required table")
        elif count <= 0:
            print(f"    ⚠ Empty optional table")
    except Exception as e:
