...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "    Fix: & '$VenvPython' -m pip install --no-cache-dir --require-hashes --only-binary cryptography -r '$(Join-Path $BackendDir "requirements.lock")'" -ForegroundColor DarkGray
    exit 1
}
