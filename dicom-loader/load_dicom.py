...
CANONICAL_HOSPITAL_NAMES = [
    "Emory University Hospital", "Piedmont Atlanta Hospital", "Grady Memorial Hospital",
    "Northside Hospital Atlanta", "WellStar Kennestone Hospital",
    "Children's Healthcare of Atlanta at Egleston",
    "Children's Healthcare of Atlanta at Scottish Rite",
    "Children's Healthcare of Atlanta at Hughes Spalding",
    "Emory Saint Joseph's Hospital", "Emory University Hospital Midtown",
...
def get_canonical_manifest(storage_account: str, credential, container: str = "synthea-output") -> dict | None:
    """Read the canonical fixture contract when this is a fixture-backed run."""
    try:
        account_url = f"https://{storage_account}.blob.core.windows.net"
        blob_service = BlobServiceClient(account_url, credential=credential)
        data = blob_service.get_blob_client(container, "_control/canonical-fixture-manifest.json").download_blob().readall()
        manifest = json.loads(data)
        if manifest.get("syntheticOnly") is not True:
            raise RuntimeError("Canonical manifest must declare syntheticOnly=true")
        return manifest
...
def process_single_patient_worker(
    ...
    stats: dict,
    tmp_base: str,
    fixture_as_of_date: str | None = None,
    ...
    try:
        ...
        if not no_fhir_mode:
            ...
            img_study_id = create_imaging_study(
                ...
                body_site, len(retagged_files), blob_base,
                f"{fixture_as_of_date}T12:00:00Z" if fixture_as_of_date else None,
            )
            with stats_lock:
...
def main():
    ...
    if os.path.exists(providers_path):
        ...
        hospital_names = [
            item["resource"]["name"]
            for item in providers.get("entry", [])
            if item.get("resource", {}).get("resourceType") == "Organization"
            and any(coding.get("code") == "hosp" for group in item.get("resource", {}).get("type", []) for coding in group.get("coding", []))
            and item.get("resource", {}).get("name")
        ]
        if not hospital_names:
            raise RuntimeError("atlanta_providers.json contains no hospital organizations")
    else:
        hospital_names = CANONICAL_HOSPITAL_NAMES.copy()
    logger.info("  Hospitals: %d available", len(hospital_names))

    # Authenticate
    azure = AzureClient(fhir_url, storage_account, dicom_container, client_id)
    logger.info("Azure authentication configured")
    fixture_manifest = get_canonical_manifest(storage_account, azure.credential)
    fixture_as_of_date = fixture_manifest.get("asOfDate") if fixture_manifest else None
    if fixture_as_of_date:
        logger.info("  Canonical fixture as-of date: %s", fixture_as_of_date)
    ...
    if fixture_manifest:
        expected = fixture_manifest.get("deviceAssignments", [])[:study_count]
        if associations != expected:
            raise RuntimeError("DICOM association mapping does not match canonical fixture manifest")
    elif not associations:
        if not no_fhir_mode:
            raise RuntimeError("No device-associated FHIR patients found")
        associations = [
            {"patientId": f"PT-MOCK-{i:04d}", "deviceId": f"MASIMO-RADIUS7-{i:04d}", "patientName": f"Jane Doe Mock {i}"}
            for i in range(1, 6)
        ]
    associations = associations[:study_count]
    logger.info("Processing %d patients", len(associations))
    ...
    if fixture_manifest and (stats["failed"] > 0 or stats["uploaded"] != len(associations) or stats["imaging_studies_created"] != len(associations)):
        raise RuntimeError(f"Canonical DICOM load incomplete: {stats}")
