...
if ($SkipEmulator) {
    $emulatorProbeOutput = @(az container show --resource-group $ResourceGroupName --name "masimo-emulator-grp" --query name -o tsv --only-show-errors 2>&1)
    $emulatorProbeExitCode = $LASTEXITCODE
    $emulatorProbeText = ($emulatorProbeOutput -join "`n").Trim()
    if ($emulatorProbeExitCode -eq 0) {
        if ([string]::IsNullOrWhiteSpace($emulatorProbeText)) {
            throw "Azure returned an empty result while inspecting telemetry emulator 'masimo-emulator-grp'"
        }
        Write-Host "  Stopping existing telemetry emulator for scaffolding-only mode..." -ForegroundColor Yellow
        az container stop --resource-group $ResourceGroupName --name $emulatorProbeText --only-show-errors | Out-Null
        if ($LASTEXITCODE -ne 0) { throw "Could not stop existing telemetry emulator '$emulatorProbeText'" }
    } elseif ($emulatorProbeExitCode -eq 3 -or $emulatorProbeText -match 'ResourceNotFound|could not be found|was not found') {
        Write-Host "  No existing telemetry emulator found." -ForegroundColor DarkGray
        $global:LASTEXITCODE = 0
    } else {
        throw "Could not inspect telemetry emulator 'masimo-emulator-grp' (az exit $emulatorProbeExitCode): $emulatorProbeText"
    }
}


$existingEmulator = if ($SkipEmulator) { $null } else { az deployment group show --resource-group $ResourceGroupName --name "emulator" --query "properties.provisioningState" -o tsv 2>$null }
$skipEmulatorAndBuild = $SkipEmulator
...
if (-not $skipEmulatorAndBuild -and -not $SkipTelemetry) {
    ...
    [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
    $imageTag = "deploy-$(Get-Date -AsUTC -Format 'yyyyMMddHHmmss')"
    ...
    if ($acrBuildExitCode -ne 0) {
        ...
        for ($attempt = 1; $attempt -le 6; $attempt++) {
            $imageExists = az acr repository show-tags --name $acrName --repository "masimo-emulator" --query "[?@=='$imageTag'] | [0]" -o tsv 2>$null
        ...
        if ($imageExists) {
            Write-Host "  ✓ Image masimo-emulator:$imageTag exists in ACR (build succeeded despite log error)" -ForegroundColor Green
        } else {
    ...
    $imageDigest = az acr manifest show-metadata --registry $acrName --name "masimo-emulator:$imageTag" --query digest -o tsv 2>$null
    if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($imageDigest)) {
        throw "Could not resolve immutable digest for masimo-emulator:$imageTag"
    }
    $fullImageTag = "$acrLoginServer/masimo-emulator@$imageDigest"
    Write-Host "  Deploying immutable image: $fullImageTag" -ForegroundColor DarkGray
    ...
    Write-Host "--- STEP 3: EMULATOR IMAGE BUILD (OMITTED FOR SCAFFOLDING) ---" -ForegroundColor DarkGray
    Write-Host "--- STEP 4: EMULATOR ACI DEPLOYMENT (OMITTED FOR SCAFFOLDING) ---" -ForegroundColor DarkGray
} else {
...
if ($SkipEmulator) {
    Write-Host "Infrastructure deployed without a telemetry producer (scaffolding-only)."
} else {
