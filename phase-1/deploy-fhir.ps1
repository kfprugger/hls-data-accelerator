...
if ($UseCachedSynthea -and $PatientCount -ne 100) {
    Write-Host "Cached canonical fixture fixes PatientCount at 100." -ForegroundColor Cyan
    $PatientCount = 100
}
if ($ReseedData) {
    $InfraOnly = $false
...
if ($script:DeploymentSubscriptionId) {
    Set-AzContext -SubscriptionId $script:DeploymentSubscriptionId -ErrorAction Stop | Out-Null
} else {
    try {
...
if ($ReseedData) {
    Write-Host "Data Reseed: True (authoritative FHIR replacement and fresh export)" -ForegroundColor Cyan
}
if ($UseCachedSynthea) {
...
if ($InfraOnly) {
    ...
    if ($SkipFhirExport) {
        Write-Host "  FHIR `$export omitted by -SkipFhirExport." -ForegroundColor DarkGray
    } else {
        # Catch-up export for callers using InfraOnly to provision an export destination.
...
if ($UseCachedSynthea) {
    ...
    az storage blob delete-batch --account-name $storageAccountName --source $containerName --auth-mode login 2>$null | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "Could not clear prior fixture blobs." }

    $localPrepackagedPath = Join-Path $fixtureRoot "prepackaged"
    az storage blob upload-batch --account-name $storageAccountName --destination $containerName --source $localPrepackagedPath --auth-mode login --overwrite --only-show-errors | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "Could not upload canonical patient bundles." }

    $manifestPath = Join-Path $fixtureRoot "canonical-fixture-manifest.json"
    az storage blob upload --account-name $storageAccountName --container-name $containerName --name "_control/canonical-fixture-manifest.json" --file $manifestPath --auth-mode login --overwrite --only-show-errors | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "Could not upload canonical fixture manifest." }
    Write-Host "  ✓ Canonical fixture bundles and manifest uploaded" -ForegroundColor Green
} else {
    ...
    # A reseed removes every prior input artifact, including stale manifests and associations.
    ...
    if ($LASTEXITCODE -ne 0) { throw "Could not clear prior Synthea output." }
...
$null = Invoke-ArmGroupDeployment `
    ...
    -ParameterArgs @(
        ...
        "aciIdentityId=$aciIdentityId",
        "aciIdentityClientId=$aciIdentityClientId",
        "reseedData=$(([bool]$ReseedData).ToString().ToLowerInvariant())",
        "--parameters", $tagsParamRef
    )
...
az container logs --resource-group $ResourceGroupName --name fhir-loader-job 2>$null | Select-Object -Last 30
    ...
    if ($ReseedData -or $UseCachedSynthea) {
        ...
        az storage blob delete-batch --account-name $storageAccountName --source "fhir-export" --auth-mode login 2>$null | Out-Null
        if ($LASTEXITCODE -ne 0) { throw "Could not clear stale FHIR export blobs." }
    }

    # FHIR $export after loader — captures Patient, Condition, Device, Observation, etc.
    if ($SkipFhirExport) {
        Write-Host "  POST-LOADER FHIR `$export skipped." -ForegroundColor DarkGray
    } elseif ($fhirServiceUrl) {
...
az container logs --resource-group $ResourceGroupName --name dicom-loader-job 2>$null | Select-Object -Last 30
    ...
    if ($SkipFhirExport) {
        Write-Host "  POST-DICOM FHIR `$export skipped." -ForegroundColor DarkGray
    } elseif ($fhirServiceUrl) {
        ...
        $postDicomExportResult = Invoke-FhirExport -ResourceGroupName $ResourceGroupName `
            -FhirServiceUrl $fhirServiceUrl `
            -Label "POST-DICOM: FHIR `$export (ImagingStudy linkage)"
