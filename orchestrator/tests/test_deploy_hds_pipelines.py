...
class FakeFabricClient:
    ...
    def request_raw(
        ...
        return self.call(method, endpoint, body, max_retries)
...
class DeployHdsCmaTests(unittest.TestCase):
    ...
    def test_clinical_export_excludes_imaging_study_owned_by_imaging(self) -> None:
        calls: list[list[str]] = []
        ...
        def runner(args, **_kwargs):
            calls.append(args)
            if args[3] == "list":
                return Result("2026/export/Patient/1.ndjson\n2026/export/ImagingStudy/2.ndjson\nImagingStudy-3.ndjson\n")
            return Result()

        removed = self.deploy_hds._remove_imaging_study_exports("stfhirtest", runner)

        self.assertEqual(removed, 2)
        delete_calls = [args for args in calls if args[3] == "delete"]
        self.assertEqual(
            [args[args.index("--name") + 1] for args in delete_calls],
            ["2026/export/ImagingStudy/2.ndjson", "ImagingStudy-3.ndjson"],
        )
    ...
    def test_skip_dicom_excludes_imaging_ingestion_but_runs_clinical_and_omop(
        ...
        result = self.deploy_hds.run(
            {"fabric_api_base": "https://fabric.test/v1", "skip_dicom": True, "skip_imaging": True},
            {"fabric_workspace_id": WORKSPACE_ID},
        )

        # Imaging ingestion is explicitly gated out when the imaging feature is disabled ...
        self.assertEqual(
            result["pipeline_results"][imaging_name], "skipped_imaging_disabled"
        )
    ...
    def test_continuation_skip_dicom_still_runs_imaging_from_existing_input(self) -> None:
        _clinical_name, imaging_name, _omop_name = self.deploy_hds.CORE_HDS_PIPELINE_NAMES
        fake_client = FakeFabricClient(
            pipeline_ids_by_name=self.pipeline_ids(include_cma=True),
            statuses_by_name=self.completed_statuses(include_cma=True),
        )
        ...
        result = self.deploy_hds.run(
            {"fabric_api_base": "https://fabric.test/v1", "skip_dicom": True, "skip_imaging": False},
            {"fabric_workspace_id": WORKSPACE_ID},
        )

        self.assertEqual(result["pipeline_results"][imaging_name], "completed")
        self.assertIn(imaging_name, self.posted_pipelines(fake_client))
