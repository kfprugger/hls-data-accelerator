...
def run(config: dict[str, Any], resources: dict[str, Any]) -> dict[str, Any]:
    ...
    skip_data_load = config.get("scaffolding_only", False) or config.get("skip_synthea", False)
    ...
    if acr_name and not skip_fhir and not skip_data_load:
        import os
    ...
    if not skip_fhir and not skip_data_load:
    ...
    if not skip_fhir and not skip_data_load:
        ...
        client.deploy_bicep(
            ...
            parameters={
                ...
                "reseedData": config.get("reseed_data", False),
            },
            tags=tags,
        )
