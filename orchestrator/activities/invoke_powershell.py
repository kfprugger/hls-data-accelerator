...
PIPELINE_SUMMARY_RE = re.compile(
    rf"[✓✗û]\s+((?:{PIPELINE_KIND_PATTERN})\s+Pipeline|Sidecar Pipeline:\s+.+?)\s+((?:COMPLETED|SUCCEEDED|INVOKED|SKIPPED|NOT STARTED|IN PROGRESS|TIMED OUT|FAILED)(?::.*)?)\s{{2,}}(\d+\.?\d*\s*(?:min|sec)|—)",
    re.IGNORECASE,
)
# Phase transition marker: @@PHASE|<number>|<label>|<stepCount>@@
PHASE_TRANSITION_RE = re.compile(r"@@PHASE\|(\d+)\|(.+?)\|(\d+)@@")
# HDS source deployment marker: @@HDS_SOURCE|name|status|detail@@
HDS_SOURCE_EVENT_RE = re.compile(r"^@@HDS_SOURCE\|([^|]+)\|([^|]+)\|(.*)@@$")
...
def _build_deploy_args(config: dict[str, Any]) -> list[str]:
    ...
    if tags:
        ...
        if config.get("scaffolding_only"):
            params.append("-ScaffoldingOnly")
        ...
        if config.get("reuse_fabric_rti"):
            params.append("-ReuseFabricRti")
        if config.get("reuse_patients"):
            params.append("-ReusePatients")
        if config.get("reseed_data"):
            params.append("-ReseedData")
        ...
        if config.get("skip_hds_source"):
            params.append("-SkipHdsSource")
    ...
    if config.get("reuse_fabric_rti"):
        args.append("-ReuseFabricRti")
    if config.get("reuse_patients"):
        args.append("-ReusePatients")
    if config.get("reseed_data"):
        args.append("-ReseedData")
    if config.get("source_resource_group"):
...
def _run_powershell(args: list[str], step_callback: Any = None, pid_callback: Any = None, instance_id: str = "") -> int:
    ...
    for line in process.stdout:
        ...
        hds_source_match = HDS_SOURCE_EVENT_RE.match(line.strip())
        if hds_source_match:
            name, status, raw_detail = hds_source_match.groups()
            detail = raw_detail
            metadata: dict[str, Any] = {}
            try:
                structured_detail = json.loads(raw_detail)
                if isinstance(structured_detail, dict) and "detail" in structured_detail:
                    detail = str(structured_detail.pop("detail") or "")
                    metadata = structured_detail
            ...
            if status == "failed" and step_callback:
                step_callback("step_failed", f"HDS Source: {name}", detail, "")
            logger.error(line) if status == "failed" else logger.info(line)
            continue
    ...
    # Stop the heartbeat before waiting. Its concurrent process.poll() can race
    # subprocess.wait() on macOS and raise OSError(EINVAL) after a successful run.
    heartbeat_stop.set()
    heartbeat_thread.join(timeout=1)
    try:
        exit_code = process.wait()
    except OSError:
        # If the child already reaped successfully, preserve its real status
        # instead of converting a process-handle race into a failed deployment.
        exit_code = process.returncode
        if exit_code is None:
            exit_code = process.poll()
        if exit_code is None:
            raise
        logger.warning("PowerShell wait raised after process completion; using exit code %d", exit_code, exc_info=True)
    logger.info("PowerShell exited with code %d", exit_code)
    return exit_code
