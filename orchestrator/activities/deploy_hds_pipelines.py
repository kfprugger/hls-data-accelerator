...
def _remove_imaging_study_exports(
    storage_account: str,
    runner: Any = subprocess.run,
) -> int:
    """Keep ImagingStudy out of clinical ingestion; DICOM imaging owns that resource type."""
    listed = runner(
        [
    ...
    if listed.returncode != 0:
        raise RuntimeError(f"Could not enumerate fhir-export before clinical ingestion: {listed.stderr}")

    pattern = re.compile(r"(^|[/_.-])ImagingStudy([/_.-]|$)", re.IGNORECASE)
    imaging_study_blobs = [name for name in listed.stdout.splitlines() if pattern.search(name)]
    for blob_name in imaging_study_blobs:
        deleted = runner(
            [
        ...
        if deleted.returncode != 0:
            raise RuntimeError(f"Could not remove duplicate clinical ImagingStudy export blob {blob_name}: {deleted.stderr}")

    logger.info(
        "Clinical export excludes %d ImagingStudy blob(s); imaging owns ImagingStudy ingestion",
        len(imaging_study_blobs),
    )
    return len(imaging_study_blobs)
...
def run(config: dict[str, Any], resources: dict[str, Any]) -> dict[str, Any]:
    ...
    storage_account = str(resources.get("fhir_storage_account") or resources.get("storage_account_name") or "")
    if config.get("skip_imaging", False):
        logger.info("Clinical FHIR export retains ImagingStudy because imaging ingestion is disabled")
    elif storage_account:
        _remove_imaging_study_exports(storage_account)
    else:
        logger.warning("FHIR storage account unavailable; could not enforce ImagingStudy ingest ownership")
    ...
    for pipeline_name in CORE_HDS_PIPELINE_NAMES[1:]:
        ...
        # Imaging ingestion is an independently selectable feature. Skipping the
        # DICOM loader alone may mean a continuation is reusing existing DICOM,
        # so only skip this pipeline when skip_imaging is explicitly selected.
        if pipeline_name == IMAGING_INGESTION_PIPELINE_NAME and config.get("skip_imaging"):
            pipeline_results[pipeline_name] = "skipped_imaging_disabled"
            logger.info(
                "Skipping imaging ingestion pipeline because imaging is disabled: %s",
                pipeline_name,
            )
            continue
