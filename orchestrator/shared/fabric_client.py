...
class FabricClient:
    ...
    def request_raw(
        ...
        for attempt in range(1, max_retries + 1):
            ...
            return response
        raise RuntimeError(f"Fabric request exhausted retries: {method} {endpoint}")
    ...
    def request_content(
        ...
        """Issue a Fabric request with an opaque payload instead of JSON."""
        url = endpoint if endpoint.startswith("http") else f"{self.api_base}{endpoint}"
        for attempt in range(1, max_retries + 1):
            headers = self._headers()
            ...
            response.raise_for_status()
            return response
        raise RuntimeError(f"Fabric content request exhausted retries: {method} {endpoint}")
    ...
    def run_notebook_job(
        ...
        """Start a notebook job and return its instance URL or identifier."""
        endpoint = (
        ...
        if parameters:
            ...
            body["parameters"] = encoded_parameters
        response = self.request_raw("POST", endpoint, body)
        ...
        if not instance_id:
            raise RuntimeError("RunNotebook response omitted Location and job id")
        return (
            f"{self.api_base}/workspaces/{workspace_id}/items/{item_id}"
