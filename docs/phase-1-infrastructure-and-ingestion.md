...
> Before running this phase, ensure your local environment and cloud subscription context meet all prerequisites. See the root [Prerequisites](../README.md#prerequisites).
...
## Step 4 — Microsoft HDS v1.4.0 Source Deployment

`Deploy-All.ps1` automatically validates and deploys the vendored Microsoft HDS and DTT source after the RTI foundation is ready. It builds the HDS/DTT wheels, adds `scipy==1.11.4`, uploads the build artifacts to `deployment_lakehouse`, imports the deployment notebooks, publishes `healthcare1_environment`, runs `master_deployer`, and validates the resulting lakehouses, notebooks, pipelines, models, and reports.

No Fabric portal action or second deployment run is required. For payload validation, retry rules, expected artifacts, and troubleshooting, see [Microsoft HDS v1.4.0 Source Deployment Guide](../fabric-rti/HDS-SETUP-GUIDE.md).
