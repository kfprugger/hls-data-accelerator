...
> Before running this phase, ensure Stage 2 is complete, Synthea is configured to generate `Claim`, `ExplanationOfBenefit`, and `Coverage` resources, and the Silver Lakehouse tables are populated. See the root [Prerequisites](../README.md#prerequisites).
