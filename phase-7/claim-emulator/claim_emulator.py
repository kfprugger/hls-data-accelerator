...
PATIENT_COUNT = max(1, int(os.getenv("PATIENT_COUNT", "100")))
...
FIXTURE_NAMESPACE = uuid.UUID(os.getenv("CANONICAL_FIXTURE_NAMESPACE", "7f3f0f14-c8f2-4a57-8d44-a1306fa8cb8e"))
random.seed(int(os.getenv("CLAIM_RANDOM_SEED", "20260701")))
...
def generate_claim(counter: int) -> dict:
    ...
    return {
        "event_id": str(uuid.uuid5(FIXTURE_NAMESPACE, f"claim-event:{counter}")),
        "event_timestamp": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "event_type": "CLAIM_SUBMITTED",
        "claim_id": f"CLM-{uuid.uuid5(FIXTURE_NAMESPACE, f'claim:{counter}').hex[:10].upper()}",
        "patient_id": str(uuid.uuid5(FIXTURE_NAMESPACE, f"patient:{patient_number - 1}")),
