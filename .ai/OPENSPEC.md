...
id: "hls-data-accelerator"
version: "1.1.0"
status: "implemented"
author: "Joey Brakefield"
updated: "2026-04-24"
repository: "https://github.com/kfprugger/hls-data-accelerator"
---
...
Single orchestrator with automated HDS source deployment.

| Phase | Steps | Duration |
|-------|-------|----------|
| Phase 1 | Azure infra → Fabric workspace → FHIR → DICOM → Fabric RTI | ~60 min |
| HDS Source | Build/upload Microsoft HDS/DTT v1.4.0 → environment → lakehouses/notebooks/pipelines → validation | ~15-45 min |
...
| HDS preflight | Local payload validation occurs before the first cloud mutation |
...
HDS Source Deployment (Automated):
  Build HDS/DTT wheels → OneLake artifacts → environment → master_deployer → validators

Phase 2 (After HDS source contract):
...
|-----------|--------|-----------|
| HDS source deployment requires paid Fabric capacity | Trial capacity cannot host the environment | Fail preflight with a paid-capacity requirement |
...
| HDS source package changes | Medium | Adapter or artifact contract changes | Versioned vendor tree, local validation, and source-contract tests |
