...
| 3b | `hds-source/Deploy-HdsSource.ps1` | Builds and deploys Microsoft HDS/DTT v1.4.0 source, publishes `healthcare1_environment`, runs validators | ~15-45 min |
...
**Total automated time:** ~2 hours, including HDS source deployment
...
1. **HDS source deployment requires paid Fabric capacity** — trial capacities cannot host the required environment and workloads
...
6. **No hosted CI/CD pipeline** — deployments run through the PowerShell/FastAPI orchestrator rather than GitHub Actions or Azure DevOps
