...
#                      to ADLS Gen2 storage. Deploy-All.ps1 then deploys Microsoft HDS source.
#   Phase 2 (-Phase2): Run after the automated HDS source contract is complete and the
#                      clinical pipeline has populated the Silver Lakehouse. Creates KQL
#                      shortcuts to Silver Patient/Condition/Device tables and replaces
#                      fn_ClinicalAlerts with the enriched version.
#
# Prerequisites:
...
if ($Phase2) {
    ...
    # Verify the automated HDS source contract before post-HDS enrichment.
    & "$PSScriptRoot/hds-source/Deploy-HdsSource.ps1" `
        -FabricWorkspaceName $FabricWorkspaceName -WorkspaceId $workspaceId -ContractOnly
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  ERROR: Microsoft HDS v1.4.0 source contract is incomplete." -ForegroundColor Red
        Write-Host "  Run Deploy-All.ps1 without -SkipHdsPipelines before Phase 2." -ForegroundColor Yellow
        exit 1
    }
    Write-Host "  ✓ HDS source contract validated" -ForegroundColor Green
...
# ============================================================================
# PREFLIGHT: HDS SOURCE CONTRACT (informational before source deployment)
# ============================================================================

Write-Host ""
Write-Host "--- PREFLIGHT: HDS Source Contract Check ---" -ForegroundColor Cyan
...
$liveItemNames = @($allItems.value | ForEach-Object { $_.displayName })
$missingHdsSource = @($hdsSourceRequired | Where-Object { $_ -notin $liveItemNames })
if ($missingHdsSource.Count -eq 0) {
    Write-Host "  ✓ HDS v1.4.0 source artifacts already present" -ForegroundColor Green
} else {
    Write-Host "  ℹ HDS source deployment has not run yet; continuing initial RTI setup." -ForegroundColor Yellow
    Write-Host "    Phase 3 will deploy Microsoft HDS/DTT v1.4.0 automatically." -ForegroundColor DarkGray
}
